  ...
  return (
    ...
    >
      <motion.svg
        ...
        role="img"
        aria-hidden
        initial={doDraw ? { opacity: 0, scale: 0.9 } : false}
        animate={doDraw ? { opacity: 1, scale: 1 } : false}
        transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
        style={{ display: 'block', flexShrink: 0, overflow: 'visible' }}
      >
        <defs>
          <linearGradient id="endulusGold" x1="0" y1="0" x2="1" y2="0.4">
            <stop offset="0%" stopColor="#a6822f" />
            <stop offset="42%" stopColor="#d9b25a" />
            <stop offset="50%" stopColor="#f7e7b8" />
            <stop offset="58%" stopColor="#d9b25a" />
            <stop offset="100%" stopColor="#a6822f" />
            {variant === 'gold' && (
              <animateTransform
        ...
        {/* resting glyph — single filled path keeps the even-odd holes intact */}
        <motion.path
          d={MARK_D}
          fill={fill}
          fillRule="evenodd"
          initial={doDraw ? { opacity: 0 } : false}
          animate={doDraw ? { opacity: 1 } : false}
      ...
      {withWordmark && (
        <span className="flex flex-col leading-none" style={{ color: textColor }}>
          <motion.span
            className="ds-display"
            style={{ fontSize: size * 0.5, fontWeight: 500, letterSpacing: '0.01em' }}
            initial={doDraw ? { opacity: 0, x: -8 } : false}
            animate={doDraw ? { opacity: 1, x: 0 } : false}
          ...
          <motion.span
            ...
            initial={doDraw ? { opacity: 0, letterSpacing: '0.2em' } : false}
            animate={doDraw ? { opacity: 0.82, letterSpacing: '0.46em' } : false}
